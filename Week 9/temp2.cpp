...
class Organism
{
    ...
    public:
        ...
        virtual char repr() = 0;
        virtual void move();
        Coordinate get_empty_adjacent_cell(Area *current_state, int x, int y);
        virtual void breed() {};
...
{
    private:
        Coordinate locate_ant(Area *current_state, int x, int y);
...
void Area::advance_time_step()
{
    ...
    Area::iterate(DOODLEBUG, "move");
    Area::iterate(ANT, "move");

    // now breeding occurs. Doodlebugs breed before ants
    Area::iterate(DOODLEBUG, "breed");
    Area::iterate(ANT, "breed");
...
Coordinate Organism::get_empty_adjacent_cell(Area *current_state, int x, int y)
{
...
void Organism::move()
{
    ...
    Coordinate valid_location = Organism::get_empty_adjacent_cell(current_state, x, y);
...
void Ant::breed()
{
    if (time_survived >= ANT_BREED_TIME)
    {
        return;
    }
    
    Coordinate valid_cell = get_empty_adjacent_cell(current_state, x, y);
    if (valid_cell.x == -1)
    {
        return;
    }
...
{
    time_survived += 1;
    Coordinate ant_location = Doodlebug::locate_ant(current_state, x, y);
...
Coordinate Doodlebug::locate_ant(Area *current_state, int doodle_x, int doodle_y)
{
...
int main()
{
    ...
    while (true)
    {
        std::cout << "Press enter to advance the time step, any other key to end.\n";
